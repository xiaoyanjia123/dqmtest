...
const char *SaveGeoTypes[] = {"REC files", "*.rec",
                              "ROOT files", "*.root",
                              0, 0
                             };

const char *OpenEventTypes[] = {"ROOT files", "*.root",
                                "rec files", "*.rec",
                                "raw files", "*.rtraw",
//#ifdef CF__ZSMSM
...
void BesClient::CreateMenuBar() {
    ...
    fMenuViewOptionHeader->AddEntry("Show Header",kM_Header_On);
    ...
    fMenuViewOptionMdc->AddEntry("Tube",   kM_Mdc_Tubes);
    ...
    fMenuView->AddPopup("Full3D",fMenuViewOptionFull3D);
...
void BesClient::HandleViewOptionMenu(Int_t id) {
    ...
    case kM_Header_On:
        if ( fMenuViewOptionHeader->IsEntryChecked(kM_Header_On) ) {
            fMenuViewOptionHeader->UnCheckEntry(kM_Header_On);
            //canvas->SetHeaderOn(kFALSE);
        } else {
            fMenuViewOptionHeader->CheckEntry(kM_Header_On);
            //canvas->SetHeaderOn(kTRUE);
        }
        break;
...
void BesClient::OpenGeoFile(TString fname) {
    ...
    if ( fname.Contains(".gdml") ) {
        fDisplay->InitGeometryFromGDML(fpath);
        fDisplay->Draw();
        return ;
    }
...
void BesClient::SaveGeoAs() {
    ...
    if (volBes) {
        ...
        TFile *f = new TFile( fi.fFilename,"RECREATE");
        volBes->Write();
        f->Close();
    }
    else {
...
void BesClient::OpenEventFile() {
    ...
    if (!gBesGeometry) {
        this->HandleError("Geometry not intialized, Load Geometry file first!");
        return;
    }
    ...
    new TGFileDialog(fClient->GetRoot(), this, kFDOpen, &fi);
    EventDir = fi.fIniDir;
    this->OpenEventFile(fi.fFilename);
}

//__________________________________________________________________

void BesClient::OpenEventFile(TString fname) {
    //
    ...
    char msg[256];
    f_evtFile_1 = fname;
    if ( fname.Contains(".root")||fname.Contains(".rtraw")||fname.Contains(".rec") ) {
        fEvent = 0;
        gEvent = 0;
        fEvent = new BesEvent();
        gEvent = fEvent;
        ...
        if (fEventTree) {
            ...
            fEventPlaySlider->SetRange(0, nEvents); // tianhl change it
            fEventPlaySlider->SetPosition(0);
        }
        else {
...
void BesClient::SavePicAs() {
    ...
    if ( fi.fFilename ) {
        ...
        if ( filename.EndsWith("gif") ||  filename.EndsWith("GIF") ) {
            ...
            new TGMsgBox(gClient->GetRoot(), this,"Error", "Please give Name and Extension of Filename! \n Valid Extensions are *.gif, *.jpg, *.ps, *.eps, *.xpm, *.png, *.tiff!",gClient->GetPicture("mb_stop_s.xpm"));
            SavePicAs();
            return;
        }
    }
...
void BesClient::SaveMyConfig() {
    ...
    ofstream fileMyConfig(fileName.c_str(), ios_base::out);

    if (fDisplay) {
        BesView *viewXY = (BesView*)fDisplay->GetPadXY()->GetView();
        if (viewXY) {
            BesStatus *status = (BesStatus*)viewXY->GetStatusXY();
            fileMyConfig << *status;
            //cout << "viewXY " << *status << endl;
        }

        BesView *viewZR = (BesView*)fDisplay->GetPadZR()->GetView();
        if (viewZR) {
            BesStatus *status = (BesStatus*)viewZR->GetStatusZR();
            fileMyConfig << *status;
            //cout << "viewZR " << *status << endl;
        }

        BesView *view3D = (BesView*)fDisplay->GetPad3D()->GetView();
        if (view3D) {
            BesStatus *status = (BesStatus*)view3D->GetStatus3D();
            fileMyConfig << *status;
...
void BesClient::LoadMyConfig() {
    ...
    ostringstream s;
    s << "The config file: " << fileName.c_str() << " does not exist ! \n Maybe You have not save your config";
    if (!FileExists(TString(fileName))) {
        this->HandleError(s.str().c_str());
        return;
    }

    ifstream fileMyConfig(fileName.c_str());
    if (fDisplay) {
        BesView *viewXY = (BesView*)fDisplay->GetPadXY()->GetView();
        if (viewXY) {
            BesStatus status;
...
void BesClient::SetAllDisplayModeButtonUnHL() {
    ...
    for (Int_t i = 0; i < 5; i++) fDisplayModeButton[i]->SetState(false);
}
